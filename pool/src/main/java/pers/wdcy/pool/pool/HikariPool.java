...
package pers.wdcy.pool.pool;

import com.codahale.metrics.MetricRegistry;
import com.codahale.metrics.health.HealthCheckRegistry;
import pers.wdcy.pool.HikariConfig;
import pers.wdcy.pool.HikariPoolMXBean;
import pers.wdcy.pool.metrics.MetricsTrackerFactory;
import pers.wdcy.pool.metrics.PoolStats;
import pers.wdcy.pool.metrics.dropwizard.CodahaleHealthChecker;
import pers.wdcy.pool.metrics.dropwizard.CodahaleMetricsTrackerFactory;
import pers.wdcy.pool.metrics.micrometer.MicrometerMetricsTrackerFactory;
import pers.wdcy.pool.util.ConcurrentBag;
import pers.wdcy.pool.util.ConcurrentBag.IBagStateListener;
import pers.wdcy.pool.util.SuspendResumeLock;
import pers.wdcy.pool.util.UtilityElf.DefaultThreadFactory;
...
import static pers.wdcy.pool.util.ClockSource.currentTime;
import static pers.wdcy.pool.util.ClockSource.elapsedDisplayString;
import static pers.wdcy.pool.util.ClockSource.elapsedMillis;
import static pers.wdcy.pool.util.ClockSource.plusMillis;
import static pers.wdcy.pool.util.ConcurrentBag.IConcurrentBagEntry.STATE_IN_USE;
import static pers.wdcy.pool.util.ConcurrentBag.IConcurrentBagEntry.STATE_NOT_IN_USE;
import static pers.wdcy.pool.util.UtilityElf.createThreadPoolExecutor;
import static pers.wdcy.pool.util.UtilityElf.quietlySleep;
import static pers.wdcy.pool.util.UtilityElf.safeIsAssignableFrom;
...
public final class HikariPool extends PoolBase implements HikariPoolMXBean, IBagStateListener
{
   ...
   private final long aliveBypassWindowMs = Long.getLong("pers.wdcy.pool.aliveBypassWindowMs", MILLISECONDS.toMillis(500));
   private final long housekeepingPeriodMs = Long.getLong("pers.wdcy.pool.housekeeping.periodMs", SECONDS.toMillis(30));
   ...
   public HikariPool(final HikariConfig config)
   {
      ...
      if (Boolean.getBoolean("pers.wdcy.pool.blockUntilFilled") && config.getInitializationFailTimeout() > 1) {
    ...
    * through {@link pers.wdcy.pool.HikariDataSource#evictConnection(Connection)} then {@code owner} is {@code true}.
    *
