...
import pers.wdcy.pool.SQLExceptionOverride;
import pers.wdcy.pool.metrics.IMetricsTracker;
import pers.wdcy.pool.pool.HikariPool.PoolInitializationException;
import pers.wdcy.pool.util.DriverDataSource;
import pers.wdcy.pool.util.PropertyElf;
import pers.wdcy.pool.util.UtilityElf;
...
import static pers.wdcy.pool.pool.ProxyConnection.*;
import static pers.wdcy.pool.util.ClockSource.*;
import static pers.wdcy.pool.util.UtilityElf.createInstance;
   ...
   void handleMBeans(final HikariPool hikariPool, final boolean register)
   {
      ...
      try {
         ...
         if ("true".equals(System.getProperty("hikaricp.jmx.register2.0"))) {
             beanConfigName = new ObjectName("pers.wdcy.pool:type=PoolConfig,name=" + poolName);
             beanPoolName = new ObjectName("pers.wdcy.pool:type=Pool,name=" + poolName);
         } else {
            beanConfigName = new ObjectName("pers.wdcy.pool:type=PoolConfig (" + poolName + ")");
            beanPoolName = new ObjectName("pers.wdcy.pool:type=Pool (" + poolName + ")");
         }
         if (register) {
